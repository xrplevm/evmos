...
def custom_evmos(tmp_path_factory):
    ...
    yield from setup_custom_evmos(
        ...
        post_init=post_init,
        chain_binary=str(path / "upgrades/genesis/bin/evmosd"),
    )


def test_cosmovisor_upgrade(custom_evmos: Evmos):
    """
    ...
    assert rsp["code"] == 0, rsp["raw_log"]
    # rsp = custom_evmos.cosmos_cli(1).gov_vote("validator", proposal_id, "yes")
    # assert rsp["code"] == 0, rsp["raw_log"]
    ...
    wait_for_port(ports.rpc_port(custom_evmos.base_port(0)))

    # test migrate keystore
    cli.migrate_keystore()

    # check basic tx works after upgrade
    wait_for_port(ports.evmrpc_port(custom_evmos.base_port(0)))
